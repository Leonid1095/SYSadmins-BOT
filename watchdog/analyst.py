...
# Каналы PLGamesBot — особый случай

Часть событий приходит не про машину, а про каналы PLGamesBot: нашего бота
забанили в чужом чате, не дали прав модератора, или мы показываем такой канал
в публичном списке. Чинится это не с нашей стороны — команду `/unban` или
`/mod` набирает сам стример у себя в чате. Подходящего действия в каталоге нет
и быть не может, поэтому предлагай только `nothing`, а в `explanation` скажи
по делу: что именно должен набрать стример и есть ли у нас способ ему это
сказать (в `context/` написано, привязан ли у него Telegram).
