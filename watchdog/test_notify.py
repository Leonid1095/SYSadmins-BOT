...
«Память: memory — swap_pct=80» и «Удалённый сервер Второй сервер» выглядят
выводом отладчика, а не сообщением человеку, и владелец перестаёт их читать — ровно то
...
class LanguageTest(unittest.TestCase):
    ...
    def test_метрики_удалённого_сервера_словами(self):
        ...
        line = self.line({"kind": "remote", "key": "Второй сервер", "to": "crit",
    ...
    def test_имя_сервера_не_склеивается_с_видом(self):
        ...
        line = self.line({"kind": "remote", "key": "Второй сервер",
                          "severity": "crit", "detail": "агент молчит"})
        self.assertTrue(line.startswith("🔴 Второй сервер"), line)
        self.assertNotIn("Удалённый сервер Второй", line)
