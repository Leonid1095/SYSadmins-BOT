...
# Адреса, с которых этот сервер приходит к агенту. Их может быть НЕСКОЛЬКО:
# сторож пробует прямой маршрут и обходной через мост, а наружу они выходят с
# разных IP. Указать один — значит закрыть фаерволом второй маршрут и потом
# гадать, почему агент «то отвечает, то нет». Перечисляются через запятую.
